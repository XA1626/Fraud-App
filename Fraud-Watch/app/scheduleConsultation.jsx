...
const ScheduleConsultation = () => {
    ...
    const auth = getAuth();
    const navigation = useNavigation(); // Ensure this hook is used within the component scope
    ...
    const consultantEmails = {
        'Consultant A': 'dexter.jones.jobs@gmail.com', // Assuming this is Dexter Jones' email
    };
    ...
    const sendEmail = (consultantEmail) => {
        const templateParams = {
            to_name: "Dexter Jones", // Name to be used in the email
            consultant_email: consultantEmail, // Email of the consultant
            user_name: auth.currentUser.displayName || auth.currentUser.email, // Name of the user
    ...
    const handleFormSubmit = async () => {
        console.log("Button Pressed"); // Debug: Check function call
        try {
            const user = auth.currentUser;
            if (!user) {
                console.log("No authenticated user found"); // Debug: Check user authentication
                Alert.alert('Error', 'User not authenticated.');
                return;
            }
    
            if (!preferredDate || !preferredTime) {
                console.log("Invalid date or time"); // Debug: Check date and time
            ...
            if (selectedDate < now) {
                console.log("Selected date/time is in the past"); // Debug: Validate date and time
            ...
            console.log("Consultation added to Firestore successfully");  // Debug: Firestore success
                ...
                [
                    {
                        text: 'Confirm',
                        onPress: () => {
                            console.log("Navigation to Dashboard");  // Debug: Navigation check
        ...
        <PaperProvider>
            <ScrollView style={styles.container}>
                ...
                <Button
                    ...
                    style={styles.button}
                    labelStyle={{ fontSize: 20 }} // Setting the font size for the button
                >
                    Submit
                </Button>
            </ScrollView>
